...
public struct CurrencyField: View {
    ...
    private var label: String {
        ...
        return formatter.string(for: value / mag) ?? ""
    }

    public init(value: Binding<Decimal>, formatter: NumberFormatter) {
        self._value = value
...
struct CurrencyInputField: UIViewRepresentable {
    ...
    class Coordinator: NSObject, UITextFieldDelegate {
        ...
        func setValue(_ value: Decimal, textField: UITextField) {
            // Update input value
            input.value = value
        ...
        func updateText(_ value: Decimal, textField: UITextField) {
            // Update field text and last valid input text
            textField.text = String(describing: value)
            lastValidInput = String(describing: value)
        }
        ...
        func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
            ...
            if string.isEmpty {
                ...
                setValue((input.value / 10), textField: textField)
            }
            return true
        }
...
//            // Multiply by 10 to shift numbers one position to the left, revert if an overflow occurs
            let inputIntVal = NSDecimalNumber(decimal: input.value).intValue
            let (multValue, multOverflow) = inputIntVal.multipliedReportingOverflow(by: 10)
            if multOverflow {
                textField.text = lastValidInput
                return
            }
...
//            // Add the new trailing digit, revert if an overflow occurs
            ...
            // Update new value
            setValue(Decimal(addValue), textField: textField)
        }
    }
}
